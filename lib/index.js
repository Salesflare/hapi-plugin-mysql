...
var _rollback = function (server, connection, callback) {
	connection.rollback(function () {
		if (process.env.NODE_ENV !== 'prod' || process.argv[2] !== 'prod'){
			server.log(logTags, 'Rolling back transaction');
		}
...
exports.register = function (server, baseOptions, next) {
	...
	pool.getConnection(function (err, connection) {
		...
		server.on('tail', function (request) {
			if (request.app.db) {
				return _endConnection(server, request.app.db, Hoek.ignore);
			}
		...
		server.on('stop', function (server) {
			pool.end(function (err) {
				if (err) {
					server.log(logTags, 'Failed to gracefully end the pool');
				}
				pool = null;
			});
		});
